<template>
  <div class="bg-white border border-slate-200 p-5 flex flex-col gap-5 shadow-sm">
    <div class="flex justify-between items-start">
      <div class="flex items-center gap-2">
        <span
          class="px-2 py-1 bg-red-500/10 text-red-500 text-[10px] font-bold uppercase tracking-wider rounded border border-red-500/20">{{ partido.estado }}</span>
        <span
          class="px-2 py-1 bg-slate-100 text-slate-600 text-[10px] font-bold uppercase tracking-wider rounded">{{ competencia.categoria }}</span>
      </div>
    ...
    <div class="flex items-center justify-between px-4 h-full max-h-44">
      <div class="flex flex-col items-center gap-3 flex-1 text-center">
        ...
        <p class="font-bold text-sm">{{ partido.clubLocal.nombre }}</p>
      </div>
      ...
      <div class="flex flex-col items-center gap-3 flex-1 text-center">
        ...
        <p class="font-bold text-sm">{{ partido.clubVisitante.nombre }}</p>
      </div>
    </div>
