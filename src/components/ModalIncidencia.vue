  ...
  <div class="fixed inset-0 z-50 backdrop-blur-xs flex items-center justify-center p-4 bg-gray-900/50">
    <div
      ...
      <div class="p-6 space-y-6 overflow-y-auto max-h-[60vh]">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="flex flex-col" v-if="form.tipoIncidencia !== 'CAMBIO'">
            ...
            <select v-model="form.idIncidenciaCatalogo"
              class="w-full rounded-lg border border-slate-200 h-12 px-4 bg-white outline-none focus:ring-2 focus:ring-blue-500">
              <option :value="null">Seleccione...</option>
              <option v-for="incidencia in motivosFiltrados" :key="incidencia.idIncidencia"
        ...
        <div v-if="form.tipoIncidencia === 'CAMBIO'"
      ...
      <div class="p-6 bg-slate-50 border-t border-slate-200 flex gap-3 justify-end">
        <button @click.prevent="cerrar" :disabled="cargando"
          class="px-5 py-2.5 rounded-lg text-slate-600 font-bold hover:bg-slate-200 disabled:opacity-50">
          Cancelar
        </button>
        <button @click.prevent="guardarIncidencia" :disabled="cargando"
          class="px-6 py-2.5 rounded-lg bg-blue-600 text-white font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 disabled:opacity-50 flex items-center gap-2">
          <span v-if="cargando"
